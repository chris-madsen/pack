...
mod tests {
    ...
    fn fwht_is_an_involution_and_preserves_energy() {
        ...
        let original_energy: f64 = original.iter().map(|v| v * v).sum();
        let mut transformed = original.clone();
        normalized_fwht(&mut transformed).unwrap();
        let transformed_energy: f64 = transformed.iter().map(|v| v * v).sum();
    ...
    fn constant_signal_has_only_dc_walsh_peak() {
        ...
        assert!(values[1..].iter().all(|v| v.abs() < 1e-9));
    }

    #[test]
    fn walsh_peaks_sorted_by_abs_coefficient_with_canonical_tie_break() {
    ...
    #[test]
    fn spectral_program_expands_walsh_basis_correctly() {
        ...
        for (position, bit) in bits.iter().enumerate() {
            ...
            assert_eq!(*bit, expected, "bit mismatch at position {position}");
        }
    }
}
